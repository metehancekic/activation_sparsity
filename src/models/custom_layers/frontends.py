...
from ...utils import l1_normalizer, DifferenceOfGaussian2d, rgb2yrgb, rgb2y, rgb2gray
from ..custom_activations import DReLU, DTReLU, TQuantization, TSQuantization, TQuantization_BPDA, TSQuantization_BPDA
from . import take_top_coeff, GaborConv2d, take_top_coeff_BPDA, LowPassConv2d, ChannelPool
...
class LP_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.training:
        #     o = o + torch.rand_like(o, device=o.device) * self.noise_level * 2 - self.noise_level
        o = self.dtrelu(o, self.lp.weight, self.beta*8.0/255)

        return o


class Shape_Layer(nn.Module):

    def __init__(self, beta=1.0, BPDA_type="maxpool_like", noise_level=8.0/255, freeze_weights=True):
        super().__init__()

        self.gabor_layer = GaborConv2d(in_channels=1, out_channels=128,
                                       padding=5, kernel_size=(11, 11))
        self.cp = ChannelPool(128)
        self.set_BPDA_type(BPDA_type)
...
class LP_Gabor_Layer_v5(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            o = o + torch.rand_like(o, device=o.device) * 8./255 - 4./255
        o = self.dtrelu(o, self.gabor_layer.weight, self.beta*8.0/255)
        o = self.to_img(o)
...
class LP_Gabor_Layer_v6(nn.Module):
    ...
    def analysis(self, x):
        ...
        o1 = self.lp(x)
        o2 = self.dtrelu(o1, self.lp.weight, self.beta*8.0/255)
